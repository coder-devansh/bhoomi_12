...
export default function AdminPortalLogin() {
  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      if (!data.user || (data.user.role !== 'admin' && data.user.email !== 'admin@bhoomisetu.com')) {
        setError('Admin access required. Please use the official admin credentials.');
        return;
      }
    ...
    <div className="min-h-screen bg-[#0F172A] flex items-center justify-center p-4">
      <div className="w-full max-w-4xl grid grid-cols-1 md:grid-cols-2 bg-white rounded-3xl shadow-2xl overflow-hidden border border-white/10">
        <div className="p-8 bg-gradient-to-br from-[#0F172A] to-[#1E293B] text-white">
          ...
          <p className="mt-3 text-white/80">Verify lawyers and issue credentials.</p>

          <div className="mt-8 space-y-2 text-sm text-white/80">
            <div>• Verify lawyers and generate passwords</div>
            <div>• Manage disputes and status</div>
            <div>• Admin-only secure access</div>
          </div>
